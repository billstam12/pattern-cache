package gr.imsi.athenarc.visual.middleware.manager.pattern;
...
import gr.imsi.athenarc.visual.middleware.cache.TimeSeriesSpanFactory;
...
import gr.imsi.athenarc.visual.middleware.manager.pattern.nfa.NFASketchSearch;
import gr.imsi.athenarc.visual.middleware.manager.pattern.util.Util;
...
public class PatternQueryManager {
    ...
    public List<List<List<Sketch>>> executeQuery(PatternQuery query) {
        ...
        List<TimeInterval> missIntervals = new ArrayList<>();
        // missIntervals.add(new TimeRange(from + 100000, from + 200000000));
        MeasureAggregationRequest measureAggregationRequest = 
            new MeasureAggregationRequest(measure, missIntervals, aggregateInterval);

        MeasureAggregationRequest measureAggregationRequest2 = 
            new MeasureAggregationRequest(2, missIntervals, aggregateInterval);
        AggregatedDataPoints aggregatedDataPoints = dataSource.getAggregatedDataPoints(
            from, to, List.of(measureAggregationRequest, measureAggregationRequest2), aggregateFunctions);
        
        // TODO: left it here.
        TimeSeriesSpanFactory.createAggregate(aggregatedDataPoints, null, null);
