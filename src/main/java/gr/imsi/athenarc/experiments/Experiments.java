...
public class Experiments<T> {
    ...
    public String queries = "./output/generate/influx/intel_lab_exp/queries_guided.txt";
        ...
        for  (int run = 0; run < runs; run ++) {
            ...
            for (int i = 0; i < sequence.size(); i += 1) {
                ...
                if (query instanceof VisualQuery){
                    ...
                    PatternUtils.executePatternQuery((PatternQuery) query, dataSource, "m4");
                } else {
    ...
    private List<TypedQuery> generateQuerySequence(Query q0, AbstractDataset dataset, boolean save) {
        ...
        if(save) sequenceGenerator.saveQueriesToFile(querySequence, Paths.get(outFolder, genFile).toString());
