 ...
 * For each aggregation interval included we store 4 longs:
 * min and max aggregate values, the value count, and the mean.
 */
public class MinMaxAggregateTimeSeriesSpan implements TimeSeriesSpan {
    ...
    public void addAggregatedDataPoint(AggregatedDataPoint aggregatedDataPoint) {
        ...
        if (stats == null) {
            throw new IllegalArgumentException("AggregatedDataPoint has no stats");
        }
        ...
        if (i < 0 || i >= size) {
            throw new IllegalStateException("Invalid index " + i
                    + " for aggregates array of size " + size
                    + " (timestamp " + aggregatedDataPoint.getTimestamp()
                    + " outside span [" + getFrom() + ", " + getTo() + "))");
        }

        double minValue = stats.getMinValue();
        double maxValue = stats.getMaxValue();
        // Sum is an optional capability — synopses without it leave the slot NaN.
        // SQLAggregateDataPointsIterator.getSum() throws IllegalStateException when
        // the sum column wasn't selected (minMax mode); AggregateStats throws
        // UnsupportedOperationException when the synopsis doesn't carry sum at all.
        double sumValue;
        try {
            sumValue = stats.getSum();
        ...
        aggregates[i * AGG_SIZE + 3] = Double.doubleToRawLongBits(sumValue);
    }
    ...
    public MinMaxAggregateTimeSeriesSpan rollUp(AggregateInterval targetInterval) {
        ...
        for (int newIdx = 0; newIdx < newSize; newIdx++) {
            ...
            // Sum and count both add across sub-intervals; mean is derived as sum/count.
            int groupTotalCount = 0;
            double groupSum = 0.0;
            for (int i = startIdx; i < endIdx; i++) {
                int cnt = (int) aggregates[i * AGG_SIZE + 2];
                groupTotalCount += cnt;
                groupSum += Double.longBitsToDouble(aggregates[i * AGG_SIZE + 3]);
            }
            final int totalCount = groupTotalCount;
            final double finalGroupSum = groupSum;
            ...
            Stats aggStats = new Stats() {
                ...
                @Override
                public double getSum() { return finalGroupSum; }
            };
    ...
    private class TimeSeriesSpanIterator implements Iterator<AggregatedDataPoint>, AggregatedDataPoint {
        ...
        public TimeSeriesSpanIterator(long queryStartTimestamp, long queryEndTimestamp) {
            this.nextIndex = getIndex(queryStartTimestamp);
            this.endIndex = queryEndTimestamp >= 0 ? getIndex(queryEndTimestamp - 1) + 1 : size;
            this.intervalMillis = aggregateInterval.toDuration().toMillis();
        }
        ...
        public AggregatedDataPoint next() {
            ...
            timestamp = from + currentIndex * intervalMillis;
            return this;
        }
        ...
        public long getTo() {
            if (currentIndex == size - 1) {
                ...
                return from + (currentIndex + 1) * intervalMillis;
            }
        }
