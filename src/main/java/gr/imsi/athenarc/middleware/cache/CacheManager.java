...
public class CacheManager {
    ...
    public static class Builder {
        ...
        private RefinementScope refinementScope = RefinementScope.SCOPED;

        /** Scoped executors only: how many doubling refinements to attempt before
         *  falling through to the full-accuracy resolver (strict-OLS for pattern,
         *  M4 for visual). 20 ≈ log2(MAX_AGG_FACTOR), so default behavior is
         *  "refine until the dataResolutionCap or target is reached". */
        ...
        /** When true, the cache lookup admits cached sub-buckets that straddle the
         *  new query's outer-bucket grid; each straddler is routed to both adjacent
         *  sketches (D2) and the chained-sketch seam dedupe in ApproxOLSSketch.combine()
         *  keeps the union tile gap-free overlap-free. */
        public Builder withRelaxedCacheReuse(boolean relaxedCacheReuse) {
        ...
        /**
         * Picks the executor for both visual and pattern queries:
         * {@link RefinementScope#SCOPED} (default; refines only the regions that
         * need it, then full-accuracy fallback on residuals) or {@link
         * RefinementScope#FULL} (single batched fetch at the chosen α + full-range
         * fallback). Applied uniformly to both query paths.
         */
        public Builder withRefinementScope(RefinementScope scope) {
            this.refinementScope = scope;
            return this;
        }

        /**
         * Scoped executors only: how many doubling refinement steps to attempt
         * before falling through to the full-accuracy resolver (strict-OLS for
         * pattern, M4 for visual). Default 20 ≈ log2(MAX_AGG_FACTOR), meaning
         * "refine until cap or target reached". Set lower to force the fallback
         * to fire earlier (experiment knob for full-accuracy vs many-regions
         * trade-off).
         */
        public Builder withMaxRefinementSteps(int steps) {
            this.maxRefinementSteps = Math.max(0, steps);
            return this;
        }
        ...
        public CacheManager build() {
            ...
            if (maxMemoryBytes != null) {
                ...
                LOG.debug("Created cache memory manager with {} MB max memory and {}% threshold",
                    maxMemoryBytes/(1024*1024), threshold*100);
            }
            
            // Create a CacheManager that uses our unified cache
            PatternQueryManager patternQueryManager =
                new PatternQueryManager(dataSource, cache, method, adaptation, initialAggregationFactor, calendarAlignment, dataReductionFactor, relaxedCacheReuse, refinementScope, maxRefinementSteps);

            VisualQueryManager visualQueryManager =
                new VisualQueryManager(dataSource, cache, dataReductionFactor, initialAggregationFactor, prefetchingFactor, method, calendarAlignment, refinementScope, maxRefinementSteps);
