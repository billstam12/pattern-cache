...
public class MatchRecognizeQueryExecutor {
     ...
     * @param method The method type (e.g., "ols")
    ...
    public static PatternQueryResults executeMatchRecognizeQuery(PatternQuery query, DataSource dataSource, String method) {
        ...
        try {
            ...
            // The MATCH_RECOGNIZE SQL scans + aggregates the full query range
            // into one bucket per time unit, then matches. Byte-equivalent cost
            // = (range / timeUnit) buckets × 5 OLS sums per bucket × 8 bytes.
            long bucketsScanned = Math.max(1L,
                    (query.getTo() - query.getFrom()) / query.getTimeUnit().toDuration().toMillis());
            results.setIoCount(5L * 8L * bucketsScanned);
     ...
     * @param method The method used (e.g., "ols")
                ...
                for (int segmentIdx = 0; segmentIdx < numSegments; segmentIdx++) {
                    ...
                    try {
                        ...
                        if (startTimestamp != null && endTimestamp != null) {
                            long startTime = DateTimeUtil.toEpochMilliUtc(startTimestamp);
                            long endTime = DateTimeUtil.toEpochMilliUtc(endTimestamp);
