...
public class DateTimeUtil {
    ...
    /**
     * Convert a {@link java.sql.Timestamp} returned by JDBC for a naive (no-TZ) SQL
     * TIMESTAMP into epoch milliseconds, treating the wall-clock fields as UTC.
     *
     * <p>{@code Timestamp.getTime()} is unsafe here: it returns ms-since-epoch as if
     * the wall-clock fields were in the JVM's local timezone, so on a non-UTC JVM the
     * result silently shifts by the local offset (e.g. Athens runs UTC+2, producing
     * a 2-hour negative drift on every bucket boundary). All JDBC-driven timestamp
     * reads in the harness should go through this helper.
     */
    public static long toEpochMilliUtc(java.sql.Timestamp timestamp) {
