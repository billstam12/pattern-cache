...
public class SqlLikeAggregatedDatapoints implements AggregatedDataPoints {

    private static final Set<String> SUPPORTED_AGGREGATE_FUNCTIONS =
            new HashSet<>(Arrays.asList("first", "last", "min", "max", "count", "sum"));
    ...
    public SqlLikeAggregatedDatapoints(SQLQueryExecutor queryExecutor, SqlDialect dialect, SQLDataset dataset,
                                       long from, long to,
    ...
    public Iterator<AggregatedDataPoint> iterator() {
        ...
        for (int measureIdx : missingIntervalsPerMeasure.keySet()) {
            ...
            if (missingIntervals == null || missingIntervals.isEmpty()) {
                ...
                selectParts.add(buildAggregateQuery(dataSourceQueries.get(dataSourceCounter++),
    ...
    private String buildAggregateQuery(String dataSourceQuery, String timestampColumn,
        ...
        if (aggFunctions.contains("first")) innerSelect.add("first_value(_value) OVER w AS first_value");
        if (aggFunctions.contains("last")) innerSelect.add("last_value(_value) OVER w AS last_value");
        ...
        if (aggFunctions.contains("min")) outerSelect.add("min(_value) AS min");
        if (aggFunctions.contains("max")) outerSelect.add("max(_value) AS max");
        if (aggFunctions.contains("count")) outerSelect.add("count(_value) AS count");
        if (aggFunctions.contains("sum")) outerSelect.add("sum(_value) AS sum");
        if (aggFunctions.contains("first")) outerSelect.add(dialect.anyValue("first_value") + " AS first");
        if (aggFunctions.contains("last")) outerSelect.add(dialect.anyValue("last_value") + " AS last");
