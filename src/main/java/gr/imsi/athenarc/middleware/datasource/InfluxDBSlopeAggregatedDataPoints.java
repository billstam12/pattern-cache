        ...
        for (int measureIdx : missingIntervalsPerMeasure.keySet()) {
            ...
            if (missingIntervals == null || missingIntervals.isEmpty()) {
                ...
                fluxQuery.append("from(bucket: \"").append(bucket).append("\")\n")
                       ...
                       .append("|> map(fn: (r) => ({\n")
                       .append("    r with\n")
                       .append("    window_idx: float(v: int(v: (uint(v: r._start) - uint(v:").append(fromDate).append(" )) / uint(v: ").append(windowInterval).append("))),\n")
                       .append("    x_norm:\n") 
                       ...
                       .append("|> map(fn: (r) => ({\n")
                       .append("      r with\n")
                       .append("      global_x: r.window_idx + r.x_norm\n" )
                       .append("}))\n")   
                       .append("|> reduce(\n")
                       .append("    fn: (r, accumulator) => ({\n")
                       .append("        sum_x: accumulator.sum_x + r.global_x,\n")
                       .append("        sum_y: accumulator.sum_y + r._value,\n")
                       .append("        sum_xy: accumulator.sum_xy + (r.global_x * r._value),\n")
                       .append("        sum_x2: accumulator.sum_x2 + math.pow(x: r.global_x, y: 2.0),\n")
                ...
                for (TimeInterval interval : missingIntervals) {
                    ...
                    fluxQuery.append("from(bucket: \"").append(bucket).append("\")\n")
                            ...
                            .append("|> map(fn: (r) => ({\n")
                            .append("    r with\n")
                            .append("    window_idx: float(v: int(v: (uint(v: r._start) - uint(v:").append(startDate).append(" )) / uint(v: ").append(windowInterval).append("))),\n")
                            .append("    x_norm:\n") 
                            .append("        (float(v: uint(v: r._time)) - float(v: uint(v: r._start)))")
                            .append("        / (float(v: uint(v: r._stop)) - float(v: uint(v: r._start)))\n")
                            .append("}))\n")     
